...
VOID Writeini(BOOL GenerateNew)
{
	...
	if (GenerateNew) {
		...
		{
			Sunrise_Dbg("Sunrise3.ini created successfully!");
			Readini();
		}
		else
			Sunrise_Dbg("Failed to create Sunrise3.ini");

	}
	else {
		if (!ini_Sunrise_W.LoadFile(PATH_INI)) {
			ini_Sunrise_W.SetBoolValue("OPTIONS", "DisableXNotify", bDisableXNotify);
			ini_Sunrise_W.SetBoolValue("OPTIONS", "EnableDevkitSockpatch", bEnableDevkitSockpatch);
	...
	{
		//Sunrise_Dbg("Changes saved to Sunrise3.ini!");
		ini_Sunrise_W.Reset();
	}
	else if (!GenerateNew)
	{
		Sunrise_Dbg("Failed to save changes to Sunrise3.ini!");
	}
}
...
VOID Readini()
{
	Sunrise_Dbg("Reading values from Sunrise3.ini");
	...
	if (!ini_Sunrise_R.LoadFile(PATH_INI)) {
		...
		Sunrise_Dbg("Error reading Sunrise3.ini. Generating new ini...");
		Writeini(TRUE);
	}
}
#pragma endregion